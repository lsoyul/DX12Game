...
struct Particle
{
    ...
    float   lifeTime;
    int     alive;
    float3  padding;    // 16bytes padding
};
...
RWStructuredBuffer<ComputeShared> g_shared : register(u1);  // 공용데이터 저장용도
...
void CS_Main(int3 threadIndex : SV_DispatchThreadID)
{
    ...
    // 모든 쓰레드가 여기까지 진행되어야 다음으로 진행
    // addCount 공유 자원 동기화 문제
    GroupMemoryBarrierWithGroupSync();  // * 동기화 배리어
