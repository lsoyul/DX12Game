...
void GraphicsDescriptorHeap::Init(uint32 count)
{
	...
	desc.NumDescriptors = count * (CBV_SRV_REGISTER_COUNT - 1); // B0는 전역으로 써야하기 때문에 -1
	...
	_groupSize = _handleSize * (CBV_SRV_REGISTER_COUNT - 1);	// B0는 전역으로 써야하기 때문에 -1
}
...
{
	// CBV_REGISTER + SRV_REGISTER => [[b0] [b1] [b2] [b3] [b4] [t0] [t1] [t2] [t3] [t4]]
	D3D12_CPU_DESCRIPTOR_HANDLE destHandle = GetCPUHandle(reg);
